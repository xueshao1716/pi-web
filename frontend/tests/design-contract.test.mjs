...
// ── 8. 主题完整性 ──
test('七套 data-theme 必需 token 完整', () => {
  ...
  const failures = []
  for (const sel of ['ink','violet','mist','kraft','shuimo','bamboo','wood']) {
...
test('资产库交互元素遵循键盘与对话框语义', () => {
  ...
  const card = readFileSync(join(ROOT, 'src/components/assets/AssetCard.tsx'), 'utf8')
  const preview = readFileSync(join(ROOT, 'src/components/assets/AssetPreview.tsx'), 'utf8')
  ...
  assert.match(card, /<button type="button"[^>]*aria-label=\{`选择 \$\{item\.name\}\`/, '资产卡片必须使用语义按钮')
  assert.match(preview, /role="dialog"/, '预览必须声明 dialog 语义')
  assert.match(preview, /aria-modal="true"/, '预览必须声明 aria-modal')
  assert.match(preview, /event\.key === 'Escape'/, '预览必须支持 Escape 关闭')
  assert.match(assets, /<AssetPreview/, '资产页必须挂载独立预览组件')
