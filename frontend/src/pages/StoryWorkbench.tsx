...
export default function StoryWorkbench() {
  ...
  const availableModels = useMemo(() => models.filter(model => supportsBeat(model, beat?.kind)), [models, beat?.kind])
  ...
  const preview = async () => {
    ...
    setBusy(true); setError('')
    try {
      const r = await StoryApi.previewRun(project.id, { sceneId: scene.id, beatId: beat.id, kind: beat.kind, model: selectedModelInfo || { provider: 'auto', id: 'auto' } })
      setProject(r.project); setProjects(items => items.map(p => p.id === r.project.id ? r.project : p))
  ...
  const run = async () => {
    ...
    setBusy(true); setError('')
    try {
      const r = await StoryApi.run(project.id, { sceneId: scene.id, beatId: beat.id, kind: beat.kind, ...(selectedModelInfo ? { model: selectedModelInfo } : {}) })
      setProject(r.project); setProjects(items => items.map(p => p.id === r.project.id ? r.project : p))
  ...
  const outputPreview = activeOutputRun?.outputAssets?.map(asset => {
    ...
    if (asset.type === 'image' && src) return <img key={asset.id} src={src} alt="生成结果" className="max-h-72 w-full rounded-pi-md object-contain bg-black/20" />
    if (asset.type === 'video' && src) return <video key={asset.id} src={src} controls preload="metadata" className="max-h-72 w-full rounded-pi-md bg-black/30" />
    if (asset.type === 'text') return <pre key={asset.id} className="max-h-72 overflow-auto whitespace-pre-wrap rounded-pi-md bg-pi-bg2/60 p-3 text-xs leading-relaxed text-pi-text">{(asset as any).text || '暂无正文'}</pre>
