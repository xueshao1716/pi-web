  ...
  return <div className="review-workbench">
    <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
      ...
      {!isLoading && !error && review?.isRepo && !review.error && <>
        ...
        <section className="review-evidence-grid"><div className="review-evidence-card"><div className="review-evidence-heading"><div><span className="review-eyebrow">AIBODY / CONTINUITY</span><h2><Layers3 className="w-4 h-4" />系统脉络</h2><p>{aibody?.principle || '宿主、母体和表现三层共同构成元枢的连续性。'}</p></div><span className="review-evidence-live">{aibody ? '已映射代码' : '读取中…'}</span></div><div className="review-layer-stack">{(aibody?.layers || []).map(layer => <LayerCard key={layer.id} layer={layer} />)}</div>{aibody?.theory?.length ? <div className="review-theory-row">{aibody.theory.map(item => <div key={item.id}><strong>{item.label}</strong><span>{item.detail}</span>{item.evidence?.length ? <code>{item.evidence.join(' · ')}</code> : null}</div>)}</div> : null}<div className="review-theory-row" aria-label="陪伴与进化边界"><div><strong>真实陪伴</strong><span>{aibody?.companionship?.continuity || '连续性未记录'}。{aibody?.companionship?.memory || '记忆边界未记录'}。{aibody?.companionship?.boundary || '边界未记录'}。</span></div><div><strong>可治理进化</strong><span>{aibody?.evolution?.mode || '进化方式未记录'}：{aibody?.evolution?.humanApproval ? '人工审批' : '审批状态未记录'}，{aibody?.evolution?.rollback ? '支持回滚' : '回滚状态未记录'}。可调整：{aibody?.evolution?.scope?.join('、') || '范围未记录'}；保护：{aibody?.evolution?.protected?.join('、') || '边界未记录'}。</span></div></div></div>
