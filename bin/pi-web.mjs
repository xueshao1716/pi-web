#!/usr/bin/env node
// 元枢全局命令：一键启动 + 打开浏览器 + 显示地址
// 兼容旧命令 pi-web；新安装优先使用 `yuanshu`
...
// 定位元枢目录：优先环境变量，其次 npm 全局包，最后默认路径
...
if (!dir) {
  console.error("❌ 未找到元枢（server.mjs）。请先安装元枢。");
  process.exit(1);
}
...
if (needInit()) {
  console.log("🛠  首次运行：自动初始化（安装元枢兼容引擎 + dsh、生成令牌、模型清单）…");
