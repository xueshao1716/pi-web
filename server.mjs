...
// ══ 专项工作台：PPT 工作室 ══
// 流程：创建独立 agent → 注入 ppt-generator 技能指令（让 agent 自己读 SKILL.md）→ 七角色协作生成 JSON → 调脚本产出 .pptx → SSE 流式进度 + 产物卡片
async function findSkillPath(name) {
  try {
    const loader = new DefaultResourceLoader({ cwd: CONFIG.cwd, agentDir: getAgentDir() });
    await loader.reload();
...
async function handleWorkshopPpt(res, body) {
  ...
  const skillPath = await findSkillPath("ppt-generator");
...
const API_ROUTES = [
  ...
  // ── 专项工作台 ──
  ["POST", "/api/workshop/ppt", async (res, req) => handleWorkshopPpt(res, await readBody(req))],
];
