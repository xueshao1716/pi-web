...
export const ENGINE_CATALOG = {
  ...
  dsh: {
    ...
    // 只回收 stdout、历史压成 8×800 字纯文本）回两个字要 26.9s，且无流式、无记忆/情绪/出图/工具轨迹。
    // 前端早就有"（暂不能主驾）"这个状态，引擎对会自动回落到次席并播报让路。
    ...
    cannot: ["不适合当主驾：每轮要新起 headless 子进程（实测冷启动数十秒）且无流式，历史只能压成近 8 条 × 800 字的纯文本摘要", "不是完整多轮大脑，不接记忆 / 出图 / 规划主循环", "依赖本机 dsh 安装和 DeepSeek 额度；厂商可卸，不能冒充元枢"],
  },
};
