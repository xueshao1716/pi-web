...
export const StoryApi = {
  ...
  previewRun: (id: string, body: { sceneId: string; beatId: string; kind: StoryGenerationRun['kind']; model: StoryGenerationRun['model']; params?: Record<string, unknown>; seed?: number; variants?: number; negative?: string; inputAssets?: StoryGenerationRun['inputAssets'] }) => api<{ project: StoryProject; run: StoryGenerationRun; context: { prompt: string; referenceIds: string[] }; plan?: StoryPlanStep[] }>(`/api/story/projects/${encodeURIComponent(id)}/run-preview`, { method: 'POST', body }),
  ...
  film: (id: string, body: { clips?: { beatId: string; runId: string }[] } = {}) => api<{ project: StoryProject; film: StoryFilm; url: string; clipCount: number; method: string; beatIds: string[]; skipped?: { beatId: string; runId: string; reason: string }[] }>(`/api/story/projects/${encodeURIComponent(id)}/film`, { method: 'POST', body, timeoutMs: 900000 }),
