...
**跨进程也已落地**（同一轮做的）：锁文件 + `O_EXCL`（各平台原子）+ 陈旧接管。
锚在 `<agentDir>/yuanshu-locks/<canonical 路径哈希>.lock`，
所有元枢进程（server / dsh 子智能体 / headless 入口）默认落到同一个目录，
所以**不 init 也一致**。挡住的是 dsh 子智能体、`bin/pi-agent.mjs`、外部脚本。

几条踩到的细节：

- **"读不出元数据" ≠ "持有者已死"**。加锁是先 `openSync(wx)` 创建、后写元数据，
  两步之间别的进程看到的是**空锁文件**。第一版把它判成陈旧直接删锁，
  结果两个进程同时持锁——**多进程互斥测试当场抓到**（加锁那条反而失败）。
  现在：元数据缺失一律按"正在初始化"处理，只有够老或同机 pid 确实不在才接管。
- 释放时**只删自己创建的**：万一期间被陈旧接管过，不能把新持有者的锁删掉。
- 等锁超时（默认 15s）**如实失败**，工具层回报"暂未写入…本次没有改动文件"，
  绝不无锁硬写。陈旧阈值 30s，且同机持有者进程已死时立刻可接管，不必等满岁数。
- 没用 `flock`（Node 核心没有）也没引第三方 native 模块——那会给"零构建安装链路"添麻烦。

`tests/unit/file-lock.test.mjs` 里真起 5 个子进程、用起跑线保证它们同时动手：
...
（第一版对照没有起跑线，5 个 spawn 间隔比 60ms 的工作窗口还长，压根不重叠，
对照证明不了任何东西；加了起跑线才有意义。）
